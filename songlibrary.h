...
public:
	...
	template <class V>
	e::EVector<T> searchMultiIndex(int idx, V val)
	{
		e::EVector<T> results;
		for (T* it = items.begin(); it != items.end(); ++it)
		{
	...
	template <class V>
	bool deleteIndex(int idx, V val)
	{
		for (T* it = items.begin(); it != items.end(); ++it)
		{
			if ((*it)[idx] == val)
...
/*
SongLibrary is a library type that stores songs
*/

class SongLibrary: public Library<Song>
{
public:
