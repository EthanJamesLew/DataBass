...
class MainControl
{
	friend class Com;
public:
	MainControl(e::EPtr<SongLibrary>& l) :lib(l){ file = new FileFormat; file->fileToProject(*lib, e::EString((char*)"songs.txt")); }

	~MainControl(){ file->projectToFile(*lib, e::EString((char*)"song1.txt")); }
